...
exports.index = function(req, res, next) {
    mysqlDB.queryBasicInfo("tb_realtime_data", function(err, data) {
				...
				if (err) {
						...
						var nodes = {};
						for(var i in data) {
								if (nodes[data[i].node_id] === undefined)
										nodes[data[i].node_id] = [];
								nodes[data[i].node_id].push(data[i].sensor_id);
						}
						// console.log(nodes);
...
//     });
// };
exports.queryRealTime = function(req, res) {
    ...
    mysqlDB.queryRealTime(queryCondition, function(err, data) {
				if(err) {
						console.log(err);
						res.render('error', { title: 'Error', err: err });
				} else {
